<div class="p-6 max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mt-5">
        ...
        <button wire:click="openCreateModal" class="bg-blue-500 hover:bg-blue-600 text-white px-5 py-3 rounded hover:text-black">
            Registrar Venda+
        </button>
    </div>
    ...
    @if ($showCreateModal)
        <div class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div class="relative bg-white border border-gray-300 rounded-lg shadow-lg p-6 z-10 max-w-4xl w-full">
                ...
                <div class="flex items-center justify-between mt-3">
                    <button wire:click="addItem" class="bg-blue-700 text-white hover:text-black px-6 py-2 rounded">
                        Adicionar Item +
                    </button>
                ...
                <div class="flex items-center justify-between mt-6">
                    <button wire:click="save" class="bg-green-600 text-white hover:text-black px-6 py-2 rounded">
                        Salvar Venda
                    </button>
                ...
                <tr>
                    ...
                    <td class="p-2 border text-center">
                        <div class="flex items-center gap-3 justify-center">
                            <button wire:click="viewDetails({{ $sale['id'] }})" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-700">
                                🔍
                            </button>
                            <button wire:click="edit({{ $sale['id'] }})" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-700">
                                📝
                            </button>
                            <button
    ...
    @if ($viewingSale)
        <div class="fixed inset-0 flex items-center justify-center z-50">
            <div class="absolute inset-0 backdrop-blur-sm"></div>
            <div class="relative bg-green-200 border border-white rounded-lg shadow-lg p-6 z-10 max-w-2xl w-full">
                <h2 class="text-xl font-bold mb-4">Detalhes da Venda</h2>

                {{-- informaçao cliente --}}
                @php
                    $digits = preg_replace('/\D/', '', $viewingSale['client']['phone'] ?? '');
                    if (strlen($digits) === 11) {
                        $phoneFmt = '('.substr($digits,0,2).') '.substr($digits,2,5).'-'.substr($digits,7,4);
                    } elseif (strlen($digits) === 10) {
                        $phoneFmt = '('.substr($digits,0,2).') '.substr($digits,2,4).'-'.substr($digits,6,4);
                    } else {
                        $phoneFmt = $viewingSale['client']['phone'] ?? '';
                    }
                @endphp
                ...
                <table class="w-full border border-collapse text-left mb-4">
                    ...
                    <tbody class="bg-white">
                        <tr>
                            ...
                            <td class="p-2 border">{{ $phoneFmt }}</td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table class="w-full table-auto border border-collapse mt-4">
                    <thead class="bg-green-300 text-black">
                        <tr>
                            <th class="p-2 border text-left">Produto</th>
                            <th class="p-2 border text-center">Quantidade</th>
                            <th class="p-2 border text-center">Preço Unitário</th>
                            <th class="p-2 border text-center">Subtotal</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <div class="mt-4 text-right">
                    <button wire:click="closeDetails" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 hover:text-black">
                        fechar
                    </button>
                </div>
            </div>
