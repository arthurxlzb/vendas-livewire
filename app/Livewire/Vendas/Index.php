...
class Index extends Component
{
    ...
    foreach ($this->items as $i => $item) {
        // busca preço do produto (ou zero, se não selecionado)
        $prod  = Product::find($item['product_id']);
        $price = $prod?->price ?? 0;

        // atualiza unit_price e subtotal na posição i
        $this->items[$i]['unit_price'] = $price;
        $qty = $item['quantity'] ?? 0;
        $this->items[$i]['subtotal']  = $price * $qty;
    }
    ...
    public function save()
    {
        ...
        DB::transaction(function () {
            $data = ['client_id' => $this->clientId, 'sale_date' => $this->saleDate, 'total' => $this->total];
            if ($this->saleId) {
